...
The footscript is a task-oriented project. The logic is based on our paper (accepted but not published yet) that validates the function of saturation in precisely and robustly representing concentration of biochem samples merely through images. However, image analysis, including extracting information and calculating, is usually done manually by researchers, whose efficiency is extremely low. That's the reason for us to automate the process by YOLO.
